    ...
    <>
      <div style={{ border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden", marginTop: 24 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
          ...
          <tbody>
            {usuarios.map((u, i) => (
              <tr key={u.id} style={{ borderBottom: "1px solid var(--border)", backgroundColor: i % 2 === 0 ? "var(--surface)" : "var(--surface-2)", opacity: carregando === u.id ? 0.5 : 1 }}>
                <td style={{ padding: "10px 16px", fontWeight: 500, color: "var(--fg)" }}>{u.name}</td>
                <td style={{ padding: "10px 16px", color: "var(--fg-muted)" }}>{u.email}</td>
                <td style={{ padding: "10px 16px" }}>
                  <span style={{
                    fontSize: 11, padding: "2px 8px", borderRadius: 10, fontWeight: 600,
                    backgroundColor: u.role === "ADMIN" ? "var(--primary-bg)" : "var(--border)",
                    color: u.role === "ADMIN" ? "var(--primary-strong)" : "var(--fg-secondary)",
                  }}>
                    {u.role}
                  </span>
                </td>
                <td style={{ padding: "10px 16px" }}>
                  <span style={{
                    fontSize: 11, padding: "2px 8px", borderRadius: 10, fontWeight: 600,
                    backgroundColor: u.isActive ? "var(--success-bg)" : "var(--danger-bg-strong)",
                  ...
                  {u.id !== sessaoId && (
                    <>
                      <button
                        ...
                        style={{ padding: "3px 10px", border: "1px solid var(--border-strong)", borderRadius: 5, background: "var(--surface)", color: "var(--fg-secondary)", cursor: "pointer", fontSize: 12, marginRight: 6 }}
                      >
                  ...
                  {redefinindo === u.id && (
                    <div style={{ display: "flex", gap: 6, alignItems: "center", marginTop: 6 }}>
                      <input
                        ...
                        style={{ padding: "4px 8px", border: "1px solid var(--border-strong)", borderRadius: 5, fontSize: 12, width: 160, color: "var(--fg)" }}
                      />
                      <button
                        onClick={() => handleRedefinirSenha(u.id)}
                        disabled={novaSenha.length < 6}
                        style={{ padding: "4px 10px", border: "none", borderRadius: 5, background: novaSenha.length < 6 ? "var(--fg-subtle)" : "var(--primary)", color: "#fff", cursor: novaSenha.length < 6 ? "not-allowed" : "pointer", fontSize: 12 }}
                      >
                        Salvar
                      </button>
                      <button
                        onClick={() => setRedefinindo(null)}
                        style={{ padding: "4px 10px", border: "1px solid var(--border-strong)", borderRadius: 5, background: "var(--surface)", color: "var(--fg-muted)", cursor: "pointer", fontSize: 12 }}
                      >
                        Cancelar
                      </button>
                    </div>
